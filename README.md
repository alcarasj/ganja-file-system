...
## API
* All requests must be sent to `ganja-web-server` - it serves as a client proxy.
...
| `/lock` |GET|Locks a lockable file in the system for the user, with option for a user-specified lock time in seconds (default is 60, max is 86400).|`fileName*`:`string`, `lockTime`:`int`|-|
